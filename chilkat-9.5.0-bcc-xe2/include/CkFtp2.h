...
class CK_VISIBLE_PUBLIC CkFtp2  : public CkMultiByteBase
{
	...
	// current remote directory. The first file/dir is at index 0, and the last one is
	// at index (NumFilesAndDirs-1)
	CkTask *GetLastAccessDtAsync(int index);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	CkTask *GetLastAccessDtByNameAsync(const char *filename);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	CkTask *GetLastAccessTimeByNameStrAsync(const char *filename);
	...
	// directory. The first file/dir is at index 0, and the last one is at index
	// (NumFilesAndDirs-1)
	CkTask *GetLastAccessTimeStrAsync(int index);
